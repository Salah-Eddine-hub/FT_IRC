...
/*   Updated: 2023/09/15 13:38:05 by iellyass         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void Server::kick(std::vector<std::string> receiveddata, int sockfd)
{
    if(receiveddata.size() < 3){
        inv_mssg(sockfd, ":irc_server 461 " + usernickMap[sockfd].get_nickname() + ' ' + receiveddata[0] + " :Not enough parameters\n");
        return ;
    }
    ...
    if(channelsMap.find(receiveddata[1]) != channelsMap.end())
    {
        ...
        else 
        {
            ...
            if (channelsMap[receiveddata[1]].get_is_member(socket)) {
                if (receiveddata.size() > 3)
                    channelsMap[receiveddata[1]].remove_the_user(socket, tmpreceiveddata1, usernickMap[socket].get_nickname(), usernickMap[sockfd].get_nickname(), receiveddata[3], usernickMap[socket].get_username());
                else
                    channelsMap[receiveddata[1]].remove_the_user(socket, tmpreceiveddata1, usernickMap[socket].get_nickname(), usernickMap[sockfd].get_nickname(), "", usernickMap[socket].get_username());
                ...
                inv_mssg(sockfd, ":irc_server 441 " + usernickMap[sockfd].get_nickname() + ' ' + receiveddata[2] + ' ' + tmpreceiveddata1 + " :They aren't on that channel\n");
            else
                inv_mssg(sockfd, ":irc_server 401 " + usernickMap[sockfd].get_nickname() + ' ' + receiveddata[2] + " :No such nick/channel\n");
        }
    }
    else
