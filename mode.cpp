...
/*   Updated: 2023/09/16 22:18:39 by sharrach         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void Server::mode(std::vector<std::string> receiveddata, int sockfd)
{
    ...
    if(channelsMap.find(receiveddata[1]) != channelsMap.end())
    {
        ...
        if(receiveddata[2] == "+i") {
            ...
            channelsMap[receiveddata[1]].broadcast(':' + usernickMap[sockfd].get_nickname() + "!~" + usernickMap[sockfd].get_username() + "@localhost MODE " + channelsMap[receiveddata[1]].get_original_channel_name() +" +i\n", -1);
        }
            ...
            channelsMap[receiveddata[1]].broadcast(':' + usernickMap[sockfd].get_nickname() + "!~" + usernickMap[sockfd].get_username() + "@localhost MODE " + channelsMap[receiveddata[1]].get_original_channel_name() +" +t\n", -1);
        }
        else if(receiveddata[2] == "-t") {
            ...
            if (receiveddata.size() > 3){
                channelsMap[receiveddata[1]].broadcast(':' + usernickMap[sockfd].get_nickname() + "!~" + usernickMap[sockfd].get_username() + "@localhost MODE " + channelsMap[receiveddata[1]].get_original_channel_name() +" -k " + channelsMap[receiveddata[1]].get_is_pwd_needed() + '\n', -1);
                channelsMap[receiveddata[1]].set_is_pwd_needed("");
            }
            else
                inv_mssg(sockfd, ":irc_server 461 " + usernickMap[sockfd].get_nickname() + " MODE -k :Not enough parameters\n");
        }
        else if(receiveddata[2] == "-l") {
            channelsMap[receiveddata[1]].set_limit(0);
            channelsMap[receiveddata[1]].broadcast(':' + usernickMap[sockfd].get_nickname() + "!~" + usernickMap[sockfd].get_username() + "@localhost MODE " + channelsMap[receiveddata[1]].get_original_channel_name() +" -l\n", -1);
        }
...
//---------------------------------------------------------------------------------------------------------------------------------------------------------------------
            ...
            if (receiveddata.size() > 3){
                if(usernickMap.find(get_sockfd(receiveddata[3])) == usernickMap.end()) {
                    ...
                    channelsMap[receiveddata[1]].broadcast(':' + usernickMap[sockfd].get_nickname() + "!~" + usernickMap[sockfd].get_username() + "@localhost MODE " + channelsMap[receiveddata[1]].get_original_channel_name() + " +o " + receiveddata[3] + '\n', -1);
                }
            }
            else
                inv_mssg(sockfd, ":irc_server 461 " + usernickMap[sockfd].get_nickname() + " MODE +o :Not enough parameters\n");
        }
//---------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
//---------------------------------------------------------------------------------------------------------------------------------------------------------------------
            ...
            if (receiveddata.size() > 3){
                ...
                channelsMap[receiveddata[1]].broadcast(':' + usernickMap[sockfd].get_nickname() + "!~" + usernickMap[sockfd].get_username() + "@localhost MODE " + channelsMap[receiveddata[1]].get_original_channel_name() +" +k " + receiveddata[3] + '\n', -1);
            }
            else
                inv_mssg(sockfd, ":irc_server 461 " + usernickMap[sockfd].get_nickname() + " MODE +k :Not enough parameters\n");
        }
            ...
            if (limit > 0)
            {
                ...
                oss << usernickMap[sockfd].get_nickname() << "!~" + usernickMap[sockfd].get_username() + "@localhost MODE "<< channelsMap[receiveddata[1]].get_original_channel_name() << " +l " << limit << " \n";
                channelsMap[receiveddata[1]].broadcast(oss.str() , -1);
            }
        }
        else 
            inv_mssg(sockfd, ":irc_server 472 " + usernickMap[sockfd].get_nickname() + ' ' + receiveddata[2] + " :is unknown mode char to me\n");
    }
    else
