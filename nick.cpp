...
int Server::is_valide_nickname(std::string &nickname, int sockfd)
{
    ...
    if (!isalpha(nickname[0]) && nickname[0] != '_') {
        if(usernickMap[sockfd].get_nickname().empty())
            inv_mssg(sockfd, ':' + localhostcheck() + " 432 "+ nickname +" :Erroneous Nickname\n");
        else
            inv_mssg(sockfd, ':' + localhostcheck() + " 432 " + usernickMap[sockfd].get_nickname() + ' ' + nickname + " :Erroneous Nickname\n");
        return 0;
    }
...
void Server::nick(std::vector<std::string> receiveddata, int sockfd) {
    ...
    if (receiveddata.size() < 2){
        if (!usernickMap[sockfd].get_nickname().empty())
            inv_mssg(sockfd, ':' + localhostcheck() + " 431 " + usernickMap[sockfd].get_nickname() + " :No nickname given\n");
        else
            inv_mssg(sockfd, ':' + localhostcheck() + " 431  :No nickname given\n");
        return ;
    }
    else{
        for (it = usernickMap.begin(); it != usernickMap.end(); it++){
            if(strtolower(it->second.get_nickname()) == strtolower(receiveddata[1]) && sockfd != get_sockfd(it->second.get_nickname())) {
                if(usernickMap[sockfd].get_nickname().empty()){
                    inv_mssg(sockfd, ':' + localhostcheck() + " 433 * " + receiveddata[1] + " :Nickname is already in use.\n");
                    return ;
                }
                else if(strtolower(usernickMap[sockfd].get_nickname()) == strtolower(receiveddata[1]))
                    return ;
                else
                    inv_mssg(sockfd, ':' + localhostcheck() + " 433 " + usernickMap[sockfd].get_nickname() + ' ' + receiveddata[1] + " :Nickname is already in use.\n");
                return ;
            }
        }
