    ...
    if (receiveddata.size() >= 3){
        ...
        if(channelsMap.find(receiveddata[1]) != channelsMap.end())
        {
            if(!channelsMap[receiveddata[1]].get_is_member(sockfd))
                inv_mssg(sockfd, ':' + localhostcheck() + " 442 " + usernickMap[sockfd].get_nickname() + ' ' + tmpreceiveddata1 + " :You're not on that channel\n");
            else if (!channelsMap[receiveddata[1]].get_is_operator(sockfd) && channelsMap[receiveddata[1]].get_is_topic_restricted())
                inv_mssg(sockfd, ':' + localhostcheck() + " 482 " + usernickMap[sockfd].get_nickname() + ' ' + receiveddata[1] + " :You're not channel operator\n");
            else{
    ...
    else if (receiveddata.size() == 2)
    {
        if(channelsMap.find(strtolower(receiveddata[1])) != channelsMap.end())
        {
            if(channelsMap[receiveddata[1]].get_channel_topic().empty())
                inv_mssg(sockfd, ':' + localhostcheck() + " 331 " + usernickMap[sockfd].get_nickname() + ' ' + channelsMap[receiveddata[1]].get_original_channel_name() + " :No topic is set.\n");
            else
                inv_mssg(sockfd, ':' + localhostcheck() + " 332 " + usernickMap[sockfd].get_nickname() + ' ' + channelsMap[receiveddata[1]].get_original_channel_name() + ' ' + channelsMap[strtolower(receiveddata[1])].get_channel_topic() + "\n");
        }
        else
            inv_mssg(sockfd, ':' + localhostcheck() + " 403 " + usernickMap[sockfd].get_nickname() + ' ' + receiveddata[1] + " :No such channel\n");
    }
    else
        inv_mssg(sockfd, ':' + localhostcheck() + " 461 " + usernickMap[sockfd].get_nickname() + " TOPIC :Not enough parameters\n");
    return;
}
