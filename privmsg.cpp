...
void Server::privmsg(std::vector<std::string> receiveddata, int sockfd){
	...
	if(receiveddata.size() == 1){
		inv_mssg(sockfd, ':' + localhostcheck() + " 411 " + usernickMap[sockfd].get_nickname() + " :No recipient given (PRIVMSG)\n");
		return ;
	}
	if(receiveddata.size() == 2)
		inv_mssg(sockfd, ':' + localhostcheck() + " 412 " + usernickMap[sockfd].get_nickname() + " :No text to send\n");
