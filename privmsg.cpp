...
/*   Updated: 2023/09/16 21:50:40 by sharrach         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void Server::privmsg(std::vector<std::string> receiveddata, int sockfd){
	...
	if(receiveddata.size() == 1){
		inv_mssg(sockfd, ":irc_server 411 " + usernickMap[sockfd].get_nickname() + " :No recipient given (PRIVMSG)\n");
		return ;
	}
	if(receiveddata.size() == 2)
		inv_mssg(sockfd, ":irc_server 412 " + usernickMap[sockfd].get_nickname() + " :No text to send\n");
